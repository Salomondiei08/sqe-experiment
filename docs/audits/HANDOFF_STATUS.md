...
- `scripts/07_verify_experiment.py` enforces this with the
  ...
  missing.
- The paper style audit is `docs/audits/PAPER_STYLE_AUDIT.json`; it now guards figure/table
...
- The figure asset audit is `docs/audits/FIGURE_ASSET_AUDIT.json`; it checks the active
  paper figures for PNG/SVG presence, minimum rendered size, brightness,
  nonblank content, and SVG text elements.
- The evidence-claim audit is `docs/audits/PAPER_EVIDENCE_CLAIM_AUDIT.json`; it blocks
  positive Pass@1, task-success, or human-validation claims while those
  readiness gates are missing. `docs/audits/SUBMISSION_READINESS.json` reports it as
  `Paper evidence-claim audit`.
- The compute-environment metadata is `docs/audits/COMPUTE_ENVIRONMENT.json`; it is not an
  experiment metric and does not invent wall-clock runtime.
- `docs/audits/LLM_USAGE_DISCLOSURE.md` is a draft disclosure for venues that require
...
- Active retrieval evidence uses 8 independently rebuilt memory-index seeds:
  `results/seed42/` through `results/seed49/`.
- The primary aggregate is `results/multiseed/multiseed_report.json`.
- The paired aggregate is `results/multiseed/multiseed_paired_tests.json`.
- `Random-Gated-Expansion` is an executed control backed by
  `results/seed*/random_budget_detailed.jsonl`.
...
- `docs/manifests/ARTIFACT_MANIFEST.json`: `n_files=242`
- `github_code_release/docs/manifests/RELEASE_MANIFEST.json`: `n_files=210`

The official `results/seed42/verification_report.json` through
`results/seed49/verification_report.json` files have also been
...
`docs/audits/SUBMISSION_READINESS.json` is current, then rebuilds `docs/manifests/ARTIFACT_MANIFEST.json`.
...
`docs/audits/SUBMISSION_READINESS.json` currently reports:
