...
|---|---|---|
| Analyze and plan first | `PAPER_CHECKLIST.md`, `docs/audits/EXPERIMENT_STATUS.md`, this audit | Done |
| Rerun/update experiments | `results/seed42/`, `results/seed43/`, `results/seed44/`, `results/seed45/`, `results/seed46/`, `results/seed47/`, `results/seed48/`, and `results/seed49/` contain baselines plus selective, always-expand, traces-only, paraphrases-only, and random-gated control on corrected 500-query evals | Done for eight independent memory-index retrieval seeds |
...
| Add real results from experiments | `results/seed42/full_report_v2.json` and LaTeX tables generated from JSON/JSONL | Done |
...
| Add experiment verifier | `scripts/07_verify_experiment.py` and `results/seed42/verification_report.json` | Done |
| Prevent hallucinated or untraceable data | `docs/audits/DATA_PROVENANCE.md` maps active evidence classes to sources; `paper/table_inventory.json` maps every active table to concrete source files; `docs/audits/CLAIM_AUDIT.md` maps claims to evidence and unsupported claims; `docs/manifests/ARTIFACT_MANIFEST.json` records checksums; verifier checks active paper/report files for forbidden synthetic-evidence references, unsupported positive claims, stale headline metrics, deprecated table inputs, legacy pilot-result references, and missing table-source files | Done |
| Improve figure/table formatting and references | `docs/audits/PAPER_STYLE_AUDIT.json` reports 4 figures, 17 tables, all captions/labels/references present, nonempty figure descriptions present, `manual_bold_count=0`, `em_dash_count=0`, and `failures=[]`; `docs/audits/FIGURE_ASSET_AUDIT.json` reports 4 readable active figures with PNG/SVG sources and no failures; verifier checks both audits and the paper generator's figure-description/float-placement behavior | Done |
| Provide conference-style layout preview | `paper/main_conference_preview.pdf`, `paper/main_conference_preview.tex`, and `docs/audits/CONFERENCE_PREVIEW_AUDIT.json`; marked as layout preview only, not official venue template and not experiment evidence | Done |
| Disclose compute environment without inventing runtime | `docs/audits/COMPUTE_ENVIRONMENT.json` records observed CPU, memory, GPU, OS, driver, and Python metadata; paper states runtime was not logged and does not estimate it | Done |
| Prepare venue-policy LLM usage disclosure | `docs/audits/LLM_USAGE_DISCLOSURE.md` contains a draft disclosure and states it is documentation only, not experiment evidence | Done |
| Prepare local GitHub release package | `github_code_release/docs/manifests/RELEASE_MANIFEST.json` reports 192 files and `contains_pass1_results=false`, `contains_human_labels=false`, `contains_synthetic_replacement_rows=false` | Done |
...
| High-quality LaTeX paper | `paper/main.tex` and rebuilt `paper/main.pdf` exist with real 500-query tables/figures, 8-seed independent memory-index aggregate with standard deviations, conservative claims, and `docs/audits/LATEX_BUILD_AUDIT.json` showing a warning-free Tectonic build | Done as retrieval paper package |
...
| Handoff status | `docs/audits/HANDOFF_STATUS.md` summarizes current verified artifacts and blockers; `scripts/07_verify_experiment.py` checks `handoff_status_missing_required_phrases=[]`; `docs/manifests/ARTIFACT_MANIFEST.json` tracks it | Done |
...
|---|---|---|
| Analyze and plan first | `PAPER_CHECKLIST.md` is retained as a deprecated planning template; `docs/audits/OBJECTIVE_AUDIT.md`, `docs/audits/DATA_PROVENANCE.md`, and `docs/audits/NEXT_EXPERIMENTS.md` document current evidence and gaps | Satisfied |
| Rerun/update experiments | `results/seed42/` through `results/seed49/` contain complete executed retrieval rows for all active methods; verifier reports `failures=[]` | Satisfied for retrieval experiments |
| Use real results only | `docs/audits/DATA_PROVENANCE.md` maps active evidence classes to JSON/JSONL sources; `paper/table_inventory.json` maps every active table to concrete source files; `docs/audits/CLAIM_AUDIT.md` lists supported and unsupported claims; `scripts/07_verify_experiment.py` rejects deprecated non-evidence tables, unsupported positive claims, invalid dataset paths, legacy 100-query pilot-result references, stale abstract headline metrics, and active tables with missing source files | Satisfied for active paper package |
| Add graphs | `paper/figures/method_overview.png`, `paper/figures/recall_at_5.png`, `paper/figures/gate_diagnostic.png`, and `paper/figures/threshold_sensitivity.png` exist; `docs/audits/FIGURE_ASSET_AUDIT.json` verifies size, brightness, nonblank content, and matching SVG sources | Satisfied |
...
| Produce LaTeX paper | `paper/main.tex` and `paper/main.pdf` exist; `scripts/31_verify_latex_clean_build.py` records `clean_build=true`, `warnings=[]`, and `failures=[]` in `docs/audits/LATEX_BUILD_AUDIT.json`; the main verifier now requires this readiness evidence | Satisfied as retrieval-paper draft |
| Professional formatting | `docs/audits/PAPER_STYLE_AUDIT.json` reports 4 figures, 17 tables, all floats captioned/labeled/referenced, nonempty figure descriptions, zero manual bold commands, zero em dash characters, no flagged hype phrases, and no failures; `docs/audits/FIGURE_ASSET_AUDIT.json` reports no figure readability failures | Satisfied |
| Conference preview | `docs/audits/CONFERENCE_PREVIEW_AUDIT.json` reports `clean_for_preview=true`, `is_official_venue_template=false`, and `is_experiment_evidence=false`; `paper/main_conference_preview.pdf` exists | Satisfied as layout preview |
| Compute disclosure | `docs/audits/COMPUTE_ENVIRONMENT.json` exists, is marked `is_experiment_metric=false`, records 96 CPU threads, 251 GiB RAM, and 8 RTX A6000 GPUs; the paper says runtime was not logged | Satisfied |
| LLM-use policy note | `docs/audits/LLM_USAGE_DISCLOSURE.md` exists and does not add experimental claims | Satisfied |
...
| Document remaining blockers | `docs/audits/BLOCKED_NEXT_ACTIONS.md` lists the Docker/admin step for Pass@1 and the required real human-label artifacts, with verifier gates; `docs/audits/SUBMISSION_READINESS.json` includes `Documented unblock steps`; `scripts/07_verify_experiment.py` cross-checks that readiness entry | Satisfied |
| Strong top-conference empirical package | `docs/audits/SUBMISSION_READINESS.json` reports `strong_submission_ready=false`, `blocking_count=2`; `pass1_harness_preflight.json` reports Docker permission is blocked; `docs/audits/PASS1_RESULTS_SCHEMA.md` is schema documentation only | Not complete |
...
- File presence checks for `paper/`, `results/seed42/`, and `docs/audits/EXPERIMENT_STATUS.md`
...
- `scripts/22_gate_feature_diagnostics.py` generated
  `results/gate_calibration/gate_feature_diagnostics.json` and
...
- `results/tokenmeasured_seed42/*_tokenmeasured500_summary.json` contains measured token usage for full 500-query reruns of Selective-QE, Always-Expand, HyDE-Traces-Only, Paraphrases-Only, and Random-Gated-Expansion
...
- `scripts/37_audit_paper_style.py` generated `docs/audits/PAPER_STYLE_AUDIT.json`; the
...
- `scripts/38_capture_compute_environment.py` generated
  `docs/audits/COMPUTE_ENVIRONMENT.json`; it is metadata only, not an experiment metric,
  and records the observed CPU, memory, GPU, OS, driver, and Python
  environment.
- `docs/audits/LLM_USAGE_DISCLOSURE.md` was added as a venue-policy disclosure draft for
...
- `scripts/36_make_conference_preview.py` generated
  ...
  and `docs/audits/CONFERENCE_PREVIEW_AUDIT.json`; the preview is not an official venue
  template and is not experiment evidence.
- `docs/audits/DATA_PROVENANCE.md` documents active paper artifact sources and marks deprecated unused diagnostics
...
- `scripts/13_make_artifact_manifest.py` generated `docs/manifests/ARTIFACT_MANIFEST.json`
  with SHA256 checksums and file sizes for active paper, result, audit, script,
  and documentation files.
- `scripts/07_verify_experiment.py` now requires `docs/manifests/ARTIFACT_MANIFEST.json` and
...
- `scripts/07_verify_experiment.py` now recomputes size and SHA256 for every
  manifest entry and fails if `docs/manifests/ARTIFACT_MANIFEST.json` is stale.
...
- `scripts/31_verify_latex_clean_build.py` now compiles `paper/main.tex` with
  Tectonic and writes `docs/audits/LATEX_BUILD_AUDIT.json`; the latest audit reports
...
- `scripts/07_verify_experiment.py` now requires the clean-build audit through
  `docs/audits/SUBMISSION_READINESS.json` and fails if LaTeX warnings or failures are
  recorded.
- `scripts/07_verify_experiment.py` completed with zero failures and zero
...
- `scripts/14_submission_readiness_check.py` generated
  `docs/audits/SUBMISSION_READINESS.json`, which records `strong_submission_ready=false`
  ...
  expansion ablations.
- `docs/audits/PASS1_RESULTS_SCHEMA.md` was added as schema documentation for future
...
- `scripts/07_verify_experiment.py` now requires `docs/audits/SUBMISSION_READINESS.json`
...
- The legacy `results/` directory is documented in `docs/audits/DATA_PROVENANCE.md` as an
...
- `scripts/06_make_paper_artifacts.py` now defaults to
  `results/seed42/` so direct runs cannot silently regenerate the
...
- `scripts/07_verify_experiment.py` now checks that
  ...
  `results/multiseed/multiseed_paired_tests.json`, and that
...
- `docs/audits/NEXT_EXPERIMENTS.md` gives concrete commands, expected evidence files, and
...
- `results_500_query_seed44_memory_seed42/verification_report.json` completed
  with zero failures and zero warnings.
- `results/multiseed/multiseed_report.json` aggregates seeds 42, 43, 44, 45, 46, 47, 48, and 49
...
- `results/multiseed/multiseed_paired_tests.json` and
...
- `results/multiseed/multiseed_gate_validation.json` and
  ...
  and 44.
- `results/gate_calibration/gate_variant_diagnostics.json` and
...
- `bash scripts/run_multiseed_artifacts.sh` was executed successfully and
  regenerated `results/multiseed/multiseed_report.json`,
  `results/multiseed/multiseed_paired_tests.json`, and
  `results/multiseed/multiseed_gate_validation.json`.
- `scripts/07_verify_experiment.py` completed with zero failures and zero
...
- `scripts/06_make_paper_artifacts.py` now reads
  `results/multiseed/multiseed_paired_tests.json` and
  `results/multiseed/multiseed_gate_validation.json` to insert the actual
...
- After correcting stale provenance, future-work text, and legacy-result guards,
  `docs/manifests/ARTIFACT_MANIFEST.json` was regenerated; the current manifest tracks 242
  files.
...
- `scripts/14_submission_readiness_check.py` now propagates the stale seed
  ...
  `docs/audits/SUBMISSION_READINESS.json`; `scripts/07_verify_experiment.py` fails if those
...
- `pass1_harness_preflight.json` now includes a `checked_at_utc` timestamp,
  `docs/audits/SUBMISSION_READINESS.json` propagates it, and `scripts/07_verify_experiment.py`
...
- `paper/main.tex` and its generator `scripts/06_make_paper_artifacts.py` now
  ...
  `docs/audits/NO_HALLUCINATED_DATA.md`, `docs/audits/DATA_PROVENANCE.md`, and `docs/audits/CLAIM_AUDIT.md` encode
...
- `scripts/19_win_loss_analysis.py` was added as a deterministic query-level
  ...
  44, 45, 46, 47, 48, and 49. It produced `results/multiseed/win_loss_analysis.json` and
...
- `docs/audits/SUBMISSION_READINESS.json` was regenerated after the stricter Pass@1 check
...
- `docs/manifests/ARTIFACT_MANIFEST.json` now tracks 201 files after adding the gate-headroom,
...
- `docs/audits/HANDOFF_STATUS.md` was added as documentation-only handoff guidance and is
...
1. No Pass@1 downstream agent task evaluation has been run.
   `docs/audits/PASS1_HARNESS_AUDIT.md` records that a nearby EvoAgentBench SWE-bench
   ...
   `docs/audits/PASS1_RESULTS_SCHEMA.md` is not evidence.
