...
"${PY}" "${ROOT}/scripts/16_gate_headroom_diagnostics.py" \
  --root "${ROOT}" \
  --seeds "42,43,44,45,46,47,48,49" \
  --output "results/gate_calibration/gate_headroom_diagnostics.json" \
...
"${PY}" "${ROOT}/scripts/28_gate_validation_paired_tests.py" \
  --root "${ROOT}" \
  --output "results/gate_calibration/gate_validation_paired_tests.json" \
...
"${PY}" "${ROOT}/scripts/06_make_paper_artifacts.py" \
  --results_dir "${ROOT}/results/seed42" \
  --paper_dir "${ROOT}/paper" \
...
# docs/manifests/ARTIFACT_MANIFEST.json stale while proving the manifest is fresh.
verify_seed \
  "${ROOT}/data_500_memory_seed42" \
  "${ROOT}/index_500_seed42" \
  "${ROOT}/results/seed42" \
