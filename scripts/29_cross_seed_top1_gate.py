"""
Cross-seed validation for a simple top-1-score expansion gate.

This diagnostic does not run retrieval and does not generate new LLM outputs.
It recombines already executed Dense-Only and Always-Expand rows. For each
held-out seed, it selects a top-1 threshold on the other completed independent
memory-index seeds and evaluates the selected gate on the held-out seed.
"""
...
def evaluate(rows, threshold, k):
    ...
    return {
        ...
        "expansion_rate": expanded_count / n,
        "deltas": deltas,
    }


def select_threshold(train_rows, k, max_expansion_rate):
    candidates = []
    for threshold in THRESHOLDS:
        metrics = evaluate(train_rows, threshold, k)
        candidates.append(
            {
                "threshold": threshold,
...
def write_table(path, rows, aggregate):
    lines = [
        ...
        r"\toprule",
        r"Test seed & Train seeds & Threshold & Gate R@5 & Dense R@5 & Delta & Expansion \\",
        r"\midrule",
    ]
    for row in rows:
...
def main(args):
    ...
    for test_seed in seeds:
        ...
        selected, candidates = select_threshold(train_rows, args.k, args.max_expansion_rate)
        metrics = evaluate(test_rows, selected["threshold"], args.k)
        reports.append(
            {
                "test_seed": test_seed,
    ...
    ci = bootstrap_ci(all_deltas, args.n_bootstrap, args.seed)
    aggregate = {
    ...
    report = {
        "artifact_type": "cross_seed_top1_gate",
        "is_pass1_result": False,
        "metric": f"Recall@{args.k}",
        "seed_family": "independent_memory",
        "procedure": (
            "Leave one independent memory-index seed out. Select a top-1-score "
            "expansion threshold on the other seeds under an expansion-rate "
            "budget, then evaluate by recombining executed Dense-Only and "
            "Always-Expand rows on the held-out seed."
        ),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="results_gate_calibration/cross_seed_top1_gate.json")
    parser.add_argument("--table_output", default="paper/tables/cross_seed_top1_gate.tex")
